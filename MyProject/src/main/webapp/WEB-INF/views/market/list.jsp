...
<div id="wrap">
	...
	<!-- 콘텐츠 영역 -->
	<div id="contents_sub">
		<h1 style="font-size: 30px; color: #000; margin-bottom: 20px;">전통시장 제품목록</h1>
			...
			<table summary="게시판 목록">
				...
				<c:if test="${ar ne null }">
					...
					<tr>
					    ...
					    <c:set var = "length" value = "${fn:length(vo.file_name)}"/>
	...
	<!-- 이미지 크기 조정 -->
						...
						<c:if test="${fn:substring(vo.file_name, length -3, length) eq 'txt'}">
							<td>${vo.file_name}</td>
						</c:if> 
...
<script>
	...
	$(function(){
		...
		$("#write_btn").bind("click", function(){
			...
			$.ajax({
				url: "m_write", //이걸 따로 지정해서 글쓰기를 하고 돌아갈 때 전통시장 목록으로 넘어가게 하자. writeController에서 코딩
				dataType: "json"
			}).done(function(data){
