...
public class WriteController {
	...
	@ResponseBody
	public Map<String, String> m_write() {
		Map<String, String> map = new Hashtable<String, String>();
	...
	public String m_writeForm() {
		MemVO mvo = (MemVO) session.getAttribute("mvo");
		String viewPath = null;
		if(mvo != null)
			viewPath = "market/write";
		else
			viewPath = "/login";
		return viewPath;
	}
	
	@RequestMapping("/m_write_ok")
	public ModelAndView m_writeOK(BbsVO vo) throws Exception {
		/*
		 * write.jsp에서 전달되는 폼의 값들(bname, subject, content, file)을 
		 * 멤버변수로 가지고 있는 BbsVO로 모두 받는다.
		 * 
		...
		*/
		...
		mv.setViewName("redirect:/market");
